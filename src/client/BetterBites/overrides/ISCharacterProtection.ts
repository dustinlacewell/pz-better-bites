...
export const ISCharacterProtection_render = function (
  this: ISCharacterProtection
) {
  ...
  const chanceWidth = getTextManager().MeasureStringX(
    UIFont.Small,
    labelChance
  );
  const biteWidth = getTextManager().MeasureStringX(UIFont.Small, labelBite);
  const scratchWidth = getTextManager().MeasureStringX(
    UIFont.Small,
    labelScratch
  );
  ...
  const chance = (def: number) =>
    math.max(0, math.min(100, luautils.round(def, 0)));
